	...
	<div style="margin:20px 0px;">
		<form action="notification.do?action=send" method="post"
			...
			<table width="600" cellpadding="4" cellspacing="0" border="0">
				<tr>
					<td width="20%">To:</td>
					<td width="80%"><input type="radio" name="category" value="0"
						checked="checked" /> All (Broadcast) <input type="radio"
						name="category" value="1" /> By UserName <input type="radio"
						name="category" value="2" /> By Alias <input type="radio"
						name="category" value="3" /> By Tag</td>
				</tr>
...
--%>
				<tr>
					...
					<span style="font-size:0.8em">ex) http://www.dokdocorea.com,
							geo:37.24,131.86, tel:111-222-3333</span></td>
				</tr>
				<tr>
					<td>&nbsp;</td>
	...
	<script type="text/javascript">
		...
		function init() {
			if ($('input[name=category]')[0].checked) {
				$('#trUsername').hide();
				$('#trAlias').hide();
				$('#trTag').hide();
			} else if ($('input[name=category]')[1].checked) {
				$('#trUsername').show();
				$('#trAlias').hide();
				$('#trTag').hide();
			} else if ($('input[name=category]')[2].checked) {
				$('#trUsername').hide();
				$('#trAlias').show();
				$('#trTag').hide();
				} else if ($('input[name=category]')[3].checked) {
				$('#trUsername').hide();
				$('#trAlias').hide();
				$('#trTag').show();
			}
